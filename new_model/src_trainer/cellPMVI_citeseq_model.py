...
from src_trainer.cellPMVAE_CITESEQ import cellPMVAE_CITESEQ

class cellPMVI_CITESEQ(UnsupervisedTrainingMixin, BaseModelClass):
    """
    cellPMVI based on totalVI
    """
    ...
    def __init__(
        ...
        **model_kwargs,):
        ...
        super(cellPMVI_CITESEQ, self).__init__(adata)
        ...
        print("Encode covariates: {}, Cat_cov: {}".format(encode_covariates, cat_cov))


        self.module = cellPMVAE_CITESEQ(n_input_genes=self.summary_stats["n_vars"],
                                        n_input_proteins=self.summary_stats["n_proteins"],
                                        n_batch=self.summary_stats["n_batch"],
                                        n_hidden=self.hparams['vae_n_hidden'],
                                        n_latent=self.hparams['vae_n_latent'],
                                        n_layers_encoder=self.hparams['vae_n_layers'],
                                        n_layers_decoder=self.hparams['vae_n_layers'],
                                        gene_likelihood='nb',
                                        dropout_rate_decoder=self.hparams['vae_dropout'],
                                        dropout_rate_encoder=self.hparams['vae_dropout'],
    ...
    def prior_predictive_sample(
        ...
        qz_m = torch.zeros(n_samples, self.hparams['vae_n_latent'])
        qz_v = torch.ones(n_samples, self.hparams['vae_n_latent'])
